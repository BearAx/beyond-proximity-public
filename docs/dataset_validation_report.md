...
|---|---:|---|---|---|---|---|---|
| `ConferenceHall` | 0 | 10, 20, 50, 100 | 0, 0, 0, 0 | `N/A` | `N/A` | `N/A` | [`ConferenceHall_view_selection.json`](dataset_validation/ConferenceHall_view_selection.json) |
| `Museume` | 0 | 10, 20, 50, 100 | 0, 0, 0, 0 | `N/A` | `N/A` | `N/A` | [`Museume_view_selection.json`](dataset_validation/Museume_view_selection.json) |
| `outdoor-drone` | 0 | 10, 20, 50, 100 | 0, 0, 0, 0 | `N/A` | `N/A` | `N/A` | [`outdoor-drone_view_selection.json`](dataset_validation/outdoor-drone_view_selection.json) |
| `outdoor-street` | 0 | 10, 20, 50, 100 | 0, 0, 0, 0 | `N/A` | `N/A` | `N/A` | [`outdoor-street_view_selection.json`](dataset_validation/outdoor-street_view_selection.json) |
| `Theater` | 0 | 10, 20, 50, 100 | 0, 0, 0, 0 | `N/A` | `N/A` | `N/A` | [`Theater_view_selection.json`](dataset_validation/Theater_view_selection.json) |

## Machine-Readable Reports

- [`ConferenceHall_geometry.json`](dataset_validation/ConferenceHall_geometry.json)
- [`Museume_geometry.json`](dataset_validation/Museume_geometry.json)
- [`outdoor-drone_geometry.json`](dataset_validation/outdoor-drone_geometry.json)
- [`outdoor-street_geometry.json`](dataset_validation/outdoor-street_geometry.json)
- [`Theater_geometry.json`](dataset_validation/Theater_geometry.json)

The existing [`default_geometry.json`](dataset_validation/default_geometry.json) remains the report for the older backend `default` scene and is not counted among the five new scenes.
...
Evidence: `docs/dataset_validation/ConferenceHall_capture_default_geometry.json`.

### Successful Pilot Evidence

The clean second attempt is recorded in `docs/dataset_validation/ConferenceHall_capture_pilot_geometry.json`. It supersedes the failed `default` attempt as capture-mechanics evidence, but it does not make the pilot or any of the five source scenes eligible for semantic or 3D evaluation.
...
|---|---:|---:|---:|---|---|---|---|---|---|---|---|---|
| `ConferenceHall-capture-pilot` | 20 | 20 | 20 | true | false | true | true | true - 20 empty templates | true - root only, incomplete | false | false | No semantic items or independent semantic/3D GT. |
| `Museume-capture` | 20 | 20 | 20 | true | false | true | true | true - 20 empty templates | true - root only, incomplete | false | false | No semantic items or independent semantic/3D GT. |
| `Theater-capture` | 20 | 20 | 20 | true | false | true | true | true - 20 empty templates | true - root only, incomplete | false | false | No semantic items or independent semantic/3D GT. |
| `outdoor-drone-capture` | 16 | 16 | 16 | true | false | true | true | true - 16 empty templates | true - root only, incomplete | false | false | Zero-depth ratio 0.403705 needs caution; no semantic items or GT. |
| `outdoor-street-capture` | 20 | 20 | 20 | true | false | true | true | true - 20 empty templates | true - root only, incomplete | false | false | No semantic items or independent semantic/3D GT. |

Machine-readable reports are `docs/dataset_validation/<scene_id>_capture_check.json`. Summary: 5 scenes checked, 5 capture scale gates passed, 0 semantic gates passed, and 0 geometry gates passed.
...
Phase C populated `views/` with 96 empty `manual` templates. Phase D generated `tree/manifest.json` and `tree/node_root.json` for each scene; every manifest is explicitly `complete=false`, with one root node and zero semantic nodes. Templates are not semantic descriptions and do not enable evaluation.

Phase E validation reports confirm 96 valid template references, zero invalid files, zero semantic items, five incomplete trees, and `semantic_eval_allowed=false` for all scenes. Phase H added captured ViewJSON v1 matching while preserving the legacy path; all reports now have `query_runner_loadable=true` and `query_runner_schema_compatible=true`. Geometry evaluation remains false because independent GT boxes/masks and predicted 3D boxes are absent.
...
|---|---:|---:|---:|---|---|---|
| `ConferenceHall-capture-pilot` | 20 | 0 | 0 | false | false | false |
| `Museume-capture` | 20 | 0 | 0 | false | false | false |
| `Theater-capture` | 20 | 0 | 0 | false | false | false |
| `outdoor-drone-capture` | 16 | 0 | 0 | false | false | false |
| `outdoor-street-capture` | 20 | 0 | 0 | false | false | false |

No semantic labels were generated. Phase B is blocked until the real images are manually annotated according to `docs/manual_semantic_annotation_queue.md` and `docs/manual_semantic_annotation_guide.md`.
