...
|---|---|---|---|---|---|---|---|
| LangSplat | No | Blocked smoke attempted | Calibrated RGB views, poses, compatible 3DGS, language features/checkpoints | Language-field relevancy and semantic localization | No checkout, `langsplat` environment, pretrained 3DGS/language features, checkpoint, or native conversion | Scene format, text query, relevancy-to-canonical localization | Literature / Baselines / Repo Notes Lead |
| ConceptGraphs | No | Blocked smoke attempted | Reliable posed RGB-D, intrinsics, segmentation/features/checkpoints | Object-centric 3D graph and geometry | No checkout, `conceptgraph` environment, checkpoints, GSA/LLaVA preprocessing, or native conversion | RGB-D loader, graph export, query, canonical output | Literature / Baselines / Repo Notes Lead |
...
No baseline smoke result is claimed. The official ConceptGraphs and LangSplat entrypoint shapes were attempted on 2026-06-23. Both were blocked before execution because their checkouts and Conda environments are absent. Required checkpoints and baseline-native preprocessing outputs are also absent. The five capture scenes now have usable RGB-D/camera inputs, but neither baseline's native dataset/model package exists.
