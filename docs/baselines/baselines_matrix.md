...
| ConceptGraphs | No | Blocked smoke attempted | Reliable posed RGB-D, intrinsics, segmentation/features/checkpoints | Object-centric 3D graph and geometry | In-repo checkout and `conceptgraph` environment missing; external checkout/checkpoints are partial and no native output exists | RGB-D loader, graph export, query, canonical output | Literature / Baselines / Repo Notes Lead |
...
ConceptGraphs remains blocked before native execution. The external checkout/checkpoints are partial, but no verified executable environment or native `native_results.json` exists. The five capture scenes now have usable RGB-D/camera inputs, but neither baseline has a completed native package for those five captured scenes.
