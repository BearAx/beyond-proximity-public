...
| Main blocker | No local LangSplat checkout/environment, compatible pretrained 3DGS/language features, checkpoint, or native scene conversion. |
...
| Main blocker | Capture geometry is usable, but no local ConceptGraphs checkout/environment/checkpoints, GSA/LLaVA preprocessing, or native Nice-SLAM Replica conversion exists. |
