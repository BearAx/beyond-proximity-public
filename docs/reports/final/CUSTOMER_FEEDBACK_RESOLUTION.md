...
| Colliding paper name | DONE | The submission title is `SemanticSplat: Graph-Pruned Semantic Search`. It preserves the established project identity and names the measured graph-pruning contribution without implying that the small instruction model is the strongest scorer. The former working name is absent from the TwinWorld manuscript source, rendered PDF, build variables, and paper figures. | `papers/twinworld/`; `scripts/check_twinworld_numbers.py` |
