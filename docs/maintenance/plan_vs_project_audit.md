...
- **Blocked / partial baselines:** Live model evaluation (OpenAI HTTP 429 `insufficient_quota` in `outputs/week3/live_gate_v1/`); cached_live (no verified live cache / no `outputs/week3/model_cache/`); ConceptGraphs smoke (no verified environment/native output); LangSplat official sofa smoke now executed with one canonical result, but five-scene comparison remains blocked; 3D IoU / geometry accuracy (no independent GT boxes); ScanNet (explicitly postponed).
...
- **Week closure vs adapted manual/stub scope:** Week 1 **partially** closed (stub CLI + design docs, not unattended capture/index). Week 2 **partially** closed (50-query stub on one scene, not ≥5 Replica automatic runs). Week 3 **partially** closed (five-scene capture + semantic index + 40 stub results; live/cached_live blocked; LangSplat sofa smoke only; ConceptGraphs blocked).
...
| Reproduce baselines (original Week 3) | PARTIALLY_DONE | `docs/baselines/baseline_status.md`, `outputs/baselines/langsplat_smoke_v1/` | LangSplat official sofa smoke produced one native and one canonical result | ConceptGraphs still has no native output; no fair five-scene baseline comparison |
