...
The pilot checker reported `scaling_allowed=true`, which authorized the completed manual capture campaign. See `docs/manual_capture_pilot.md` for the original gate.
...
Batch result: 5/5 capture-valid, 0/5 semantic-evaluation-ready, and 0/5 geometry-evaluation-ready. Phase C created 96 `manual` templates, but zero are filled. Phase D created five root-only trees with `complete=false` and zero semantic nodes; therefore no usable semantic index exists yet.

Phase E reports are stored as `docs/dataset_validation/<scene_id>_semantic_index.json`. All 96 templates pass structural/reference validation. Phase H added captured ViewJSON v1 matching, so `query_runner_loadable=true` and `query_runner_schema_compatible=true`; both evaluation flags remain false because summaries/items are empty and trees are incomplete.
